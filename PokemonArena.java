...
public class PokemonArena{
	...
	public static boolean turn, stunned, play;
	...
	public static Pokemon chooseUser(){
	...
	public static void playgame(){
		while(true){
			...
			if(pokemonTeam.size() == 0){
				System.out.println("You couldnt deafeat all the pokemons, there were " + PokemonArray.size() + " pokemons remaining.");
				break;
			}
			if(PokemonArray.size() == 0){
				System.out.println("Congratulations you defeated all pokemons you are now crowned Trainer Supreme!!!!!");
				break;
			}
		}// running the game
	}
	...
	public static void battle(){
				...
				if(choice == 1){
					...
					while(true){
						...
						if(choose + 1> 0 && choose  < user.nums_attack && user.attack.get(choose).energy < user.energy){
							System.out.println("\n" + "-----BATTLE-----" + "\n");
							System.out.println(user.name + " attacked " + enemy.name + " with " + user.attack.get(choice).name);
		...
		if(turn == false){
			if(enemy.hp != 0){
				if(enemy.energy < enemy.leastEnergy){
					System.out.println(enemy.name + " didn't have enough energy for this round");
					turn = true;
				}
				else{
					while(true){
						...
						System.out.println("\n" + "-----BATTLE-----" + "\n");
						System.out.println(enemy.name + " attacks " + user.name + " with " + enemy.attack.get(e_attpick).name + "\n");
